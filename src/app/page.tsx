...
import { ShieldCheck, Search, Droplets, Activity, Share2, AlertTriangle, Clock, Users } from 'lucide-react'
  ...
  return (
    <main className="min-h-screen flex flex-col items-center py-16 px-6 relative overflow-hidden bg-[#080808] text-[#E8E8E2]">
      ...
      <div 
        ...
        style={{
          background: 'radial-gradient(ellipse, rgba(0,255,133,0.05) 0%, transparent 70%)'
        }}
      />

      <div className="relative z-10 w-full max-w-[860px] flex flex-col gap-9">
        
        <header className="flex flex-col items-center gap-4.5 text-center">
          <div className="w-16 h-16 rounded-2xl border border-[#00FF85]/30 bg-[#00FF85]/5 flex items-center justify-center text-[#00FF85] relative">
          ...
          <div>
            ...
            <p className="text-[15px] text-[#666660] mt-2">
              Solana meme coin rug-risk scanner · powered by Birdeye
            </p>
          </div>
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-[#00FF85]/20 bg-[#00FF85]/5 text-[11px] text-[#00FF85] font-mono tracking-widest mt-1">
        ...
        <div className="border border-white/5 rounded-2xl bg-white/[0.02] p-1.5 backdrop-blur-md">
          <div className="flex gap-1.5">
            ...
            >
              {loading ? <span className="w-4 h-4 border-2 border-[#080808]/30 border-t-[#080808] rounded-full animate-spin" /> : <Search size={16} />}
              {loading ? 'Scanning...' : 'Scan'}
            </Button>
          </div>
          <div className="flex flex-wrap gap-2 pt-3 pb-1 px-2 items-center">
            <span className="text-[11px] text-[#44443E] font-mono tracking-widest pr-1">TRY →</span>
            {demoTokens.map((t) => (
              <button 
                key={t.label} 
                ...
                onClick={() => { setAddress(t.address); scan(t.address) }}
              >
                {t.label}
              </button>
            ))}
        ...
        {error && (
          <Alert variant="destructive" className="border-red-500/25 bg-red-500/5 text-red-400 font-mono text-[13px] p-4 rounded-xl">
        ...
        {result && cfg && !loading && (
          <div ref={resultRef} className="animate-in fade-in slide-in-from-bottom-4 duration-400 p-1">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <StatCard 
                ...
                value={result.meta ? `${result.meta.top10Ownership.toFixed(1)}%` : 'N/A'}
